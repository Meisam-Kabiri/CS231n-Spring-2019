...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss_i = -correct_scores + np.log(np.sum(np.exp(scores), axis =1, keepdims = True))
        loss = np.mean(loss_i)+reg*(W1**2).sum()+reg*(W2**2).sum()
        ...
        grads['W2']= z.T.dot(dscores)/N+2*reg*W2  
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            indx=np.random.choice(range(num_train),batch_size)
            X_batch=X[indx,:]
            y_batch=y[indx]
