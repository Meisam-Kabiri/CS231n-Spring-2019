...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
              hout, cache_rnn = rnn_forward(embed_captions, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
              hout, cache_rnn = lstm_forward(embed_captions, h0, Wx, Wh, b)
            
        scores, cache_scores = temporal_affine_forward (hout, W_vocab, b_vocab)
        loss, d_temp_loss = temporal_softmax_loss (scores, captions_out, mask)   
        
        #Backward pass
        
        dscores, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(d_temp_loss, cache_scores)
        if self.cell_type == 'rnn':
